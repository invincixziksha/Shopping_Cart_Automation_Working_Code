...
import pages.Shopping_Cart_Automation_Pages;
import pages.StartupPage;
import testBase.AppTestBase;
import testdata.LocatorsFactory;

public class Shopping_Cart_Automation_L2_TestCases extends AppTestBase
{
	Map<String, String> configData;
	Map<String, String> loginCredentials;
	String expectedDataFilePath = testDataFilePath+"expected_data.json";
	StartupPage startupPage;
	Shopping_Cart_Automation_Pages practiceAutomationInstance;
	...
	// add some test here to make a flow here....
		...
		@Test(priority = 12, groups = {"sanity"}, description="Verify Coupon code field and Apply Coupon button is present")
		...
		@Test(priority = 13, groups = {"sanity"}, description="Check if we have price defined after product name")
		...
		@Test(priority = 14, groups = {"sanity"}, description="Check if we have quantity defined after product price.")
		...
		@Test(priority = 15, groups = {"sanity"}, description="Check if we have total defined is present after quantity.")
		...
		@Test(priority = 16, groups = {"sanity"}, description="Validate if we have an option to remove an added product or not and remove the product.")
		...
		@Test(priority = 17, groups = {"sanity"}, description="Validate if we get a message after removing an item with its name or not.")
		...
		@Test(priority = 18, groups = {"sanity"}, description="Go to the Home Icon and click on selenium ruby then click on add to basket then click on view basket to add the product")
		...
		@Test(priority = 19, groups = {"sanity"}, description="Validate if we have a “Proceed to checkout” button or not.")
